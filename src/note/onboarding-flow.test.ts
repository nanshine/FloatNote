...
function setup(project = true) {
  ...
  const controller = createOnboardingController({
    ...
    openAssistant: async () => controller.assistantOpened(), captureShortcut: () => "⌥⌘C",
  });
...
describe("onboarding interaction", () => {
  it("starts capture for an existing project, acknowledges actions, and finishes only on explicit completion", async () => {
    ...
    await click("完成引导"); expect(mocks.state.status).toBe("completed"); expect(title()).toBeUndefined();
  });
  ...
  it("uses a two-step document path instead of dismissing the tour", async () => {
    const { controller, createPiece } = setup(false); await controller.start();
    expect(title()).toBe("从这里开始写作");
    expect(document.querySelector(".onboarding-progress")?.textContent).toContain("1 / 2");
